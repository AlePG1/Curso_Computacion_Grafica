...
* 422066992
* Práctica 4: Modelado jer�rquico       
* Fecha de entrega : 01 marzo de 2025
*/
...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Previo 4 Garcia Gonzalez Alejandro", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, glm::radians(codo), glm::vec3(0.0f, 1.0f, 0.0f)); //hombro
		...
		model = glm::rotate(model, glm::radians(muneca), glm::vec3(1.0f, 0.0f, 0.0f)); //hombro
		...
		model = glm::rotate(model, glm::radians(dedo1), glm::vec3(0.0f, 0.0f, 1.0f)); //hombro
		...
		//Dedo 2A
		model = glm::translate(modelTemp2, glm::vec3(0.25f, -0.375f, -0.375f));
...
void Inputs(GLFWwindow* window) {
	...
	if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS)
		if (codo < 35) //ángulo de movimiento
			codo += 0.18f;
