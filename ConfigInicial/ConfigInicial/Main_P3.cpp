...
int main() {
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Proyecciones y transformaciones basicas García González Alejadro", nullptr, nullptr);
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// CAMBIO: Cámara un poco más arriba y atrás
		view = glm::translate(view, glm::vec3(2.0f, 3.0f, -10.0f));

		// CAMBIO: Rotación más grande y en un eje inclinado (1.0, 0.5, 0.0)
		model = glm::rotate(model, 2.5f, glm::vec3(1.0f, 0.5f, 0.0f));

		// CAMBIO: Escala con forma alargada (1.5, 1.0, 4.0)
		...
		// CAMBIO: Lo movemos hacia la izquierda, abajo y atrás
		model = glm::translate(model, glm::vec3(-2.0f, -1.0f, -5.0f));

		// CAMBIO: Rotación de 1.5 rad (~86°) alrededor de un eje complejo (3.0f, 2.0f, 1.0f)
		model = glm::rotate(model, 1.5f, glm::vec3(3.0f, 2.0f, 1.0f));

		// CAMBIO: Escala muy distinta (4.0f, 2.5f, 0.5f)
